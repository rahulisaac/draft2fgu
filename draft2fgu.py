...
def convert_to_fgu(filename, input_path, output_path, portal_width, portal_length):  
    ...
    for portal in file['portals']:
        portal_dict = dict()
        
        if portal['closed'] == True:
            portal_dict['type'] = 'door'
        elif portal['closed'] == False:
            portal_dict['type'] = 'window'

        portal_dict['x'] = [coord['x'] for coord in portal['bounds']]
        portal_dict['y'] = [coord['y'] for coord in portal['bounds']]
        
        portal_dict['closed'] = portal['closed']
        portal_dict['rotation'] = portal['rotation']

        occular_list.append(portal_dict)
    ...
    for occular in occular_list:
        ...
        if occular['type'] == 'door':
            ...
            x_trans = [x_trans[0]-epsilon_w_x, x_trans[1]-epsilon_w_x, x_trans[1]+epsilon_w_x, x_trans[0]+epsilon_w_x]
            y_trans = [y_trans[0]-epsilon_w_y, y_trans[1]-epsilon_w_y, y_trans[1]+epsilon_w_y, y_trans[0]+epsilon_w_y]
            
        if occular['type'] == 'window':
            epsilon_w_x = epsilon_w * sin(occular['rotation'])
            epsilon_w_y = epsilon_w * cos(occular['rotation'])
    ...
    for id_, object_ in enumerate(occular_list):
        ...
        elif object_['type'] == 'window':
            window = Element('window')
            window.text = 'true'
            occluder.append(window)
